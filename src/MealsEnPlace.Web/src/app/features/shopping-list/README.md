...
- **ShoppingListPageComponent** — Table showing category, ingredient name, quantity, and unit. Loads the active meal plan first, then its shopping list.
  ...
  - "Push to Todoist" button is disabled until the Todoist integration is configured — save a token on the Settings page (MEP-035); the legacy `Todoist:Token` user secret still works as a fallback. On push, a snackbar reports counts of created / updated / closed / unchanged tasks.
