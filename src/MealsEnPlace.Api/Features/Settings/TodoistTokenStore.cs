...
/// <summary>
/// File-backed Todoist API token store. The plaintext token is DataProtection-encrypted
...
/// committed to source control. Uses a distinct DataProtection purpose string
/// from the Claude token store so ciphertexts are not interchangeable.
/// </summary>
public sealed class TodoistTokenStore : ITodoistTokenStore
{
    private const string ProtectorPurpose = "MealsEnPlace.TodoistToken.v1";

    private readonly IDataProtector protector;
    private readonly string tokenFilePath;

    public TodoistTokenStore(IDataProtectionProvider dataProtectionProvider, TodoistTokenStoreOptions options)
    {
