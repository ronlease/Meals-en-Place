...
/// <summary>
/// Persists the user's Todoist personal API token to an encrypted file outside
/// the repo, using ASP.NET DataProtection. The raw token is never returned from
/// any API response — only the caller that explicitly reads the token (the
/// Todoist HTTP client and the Test Connection endpoint) ever sees the plaintext.
/// </summary>
public interface ITodoistTokenStore
{
    ...
    /// <summary>Persists <paramref name="token"/> as the current Todoist API token.</summary>
