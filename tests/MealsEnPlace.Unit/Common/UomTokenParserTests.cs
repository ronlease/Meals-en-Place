...
public class UomTokenParserTests
{
    [Fact]
    public void Parse_IntegerWithUnit_SplitsCleanly()
    {
        var (quantity, token) = UomTokenParser.Parse("2 cups");
    ...
    {
        var (quantity, token) = UomTokenParser.Parse("1.5 tbsp");
    ...
    {
        var (quantity, token) = UomTokenParser.Parse("1/2 cup");
    ...
    {
        var (quantity, token) = UomTokenParser.Parse("1/4 tsp");
    ...
    {
        var (quantity, token) = UomTokenParser.Parse("500g");
    ...
    {
        var (quantity, token) = UomTokenParser.Parse("a knob");
    ...
    public void Parse_FractionWithZeroDenominator_ReturnsZeroAndDoesNotThrow()
    {
        Action act = () =>
        {
            var (quantity, _) = UomTokenParser.Parse("1/0 cup");
            quantity.Should().Be(0m);
        };
    ...
    {
        var (quantity, token) = UomTokenParser.Parse(string.Empty);
